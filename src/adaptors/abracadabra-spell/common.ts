...
const sdk = require('@defillama/sdk') as typeof import('@defillama/sdk');
const {
  utils: { formatUnits },
  BigNumber,
...
async function getRewardTokens(
  chain: string,
  farms: MultiRewardFarms[string]
): Promise<{ [farm: string]: string[] }> {
  const farmEntries = Object.entries(farms);

  const rewardTokens = farmEntries.map(
    ([address, { pool }]) => [address, pool.rewardTokens ?? []] as const
  );

  const rewardTokenLengthCalls = farmEntries
    .map(([target, { pool }], farmIndex) => {
  ...
  const rewardTokenLengthResults = await sdk.api.abi
    .multiCall({
      abi: MULTI_REWARDS_STAKING.find(
        ({ name }) => name === 'getRewardTokenLength'
      ),
      calls: rewardTokenLengthCalls.map(({ target }) => ({ target })),
      chain,
    })
    .then((call) =>
      call.output.map((x, callIndex) => ({
  ...
  const rewardTokenResults = await sdk.api.abi
    .multiCall({
...
function getStakingTokens(
  ...
  return sdk.api.abi
    ...
    .then((call) =>
      call.output.map((x, callIndex) => [farmAddresses[callIndex], x.output])
    )
  ...
  const pools = await Promise.all(
    Object.entries(farms).map(async ([chain, chainFarms]) => {
      ...
      const [
        ...
        stakingTokensPromise
          .then((stakingTokens) =>
            sdk.api.abi.multiCall({
              abi: 'erc20:decimals',
              calls: Array.from(new Set(Object.values(stakingTokens))).map(
                (target) => ({ target })
              ),
              chain,
            })
        ...
        rewardTokensPromise.then((rewardTokens) =>
          sdk.api.abi
            .multiCall({
              abi: MULTI_REWARDS_STAKING.find(
                ({ name }) => name === 'rewardData'
              ),
              calls: Object.entries(rewardTokens).flatMap(
                ([target, farmRewardTokens]) =>
                  farmRewardTokens.map((rewardToken) => ({
                    target,
                    params: [rewardToken],
                  }))
              ),
        ...
        sdk.api.abi
          ...
          .then(Object.fromEntries),
        Promise.all([rewardTokensPromise, stakingTokensPromise]).then(
          ([rewardTokens, stakingTokens]) =>
            utils.getPrices(
              [
            ...
            async ([farmAddress, { stakingTokenPool, pool }]) => {
              ...
              if (stakingTokenPrice === undefined) {
                const underlyingTokens =
                  ...
                  yieldPools.find(({ pool }) => pool === stakingTokenPool)
                    ?.underlyingTokens;
                ...
                const [
                  ...
                  sdk.api.abi
                    .multiCall({
                      abi: 'erc20:balanceOf',
                      calls: underlyingTokens.map((underlyingToken) => ({
                        target: underlyingToken,
                        params: [stakingTokens[farmAddress]],
                      })),
                      chain,
                    })
                ...
                for (const [
                  index,
                  underlyingToken,
                ] of underlyingTokens.entries()) {
                  const underlyingTokenPrice =
                    underlyingTokenPrices.pricesByAddress[
                  ...
                  const underlyingTokenBalance = Number(
                    formatUnits(
                      underlyingTokenBalances[index],
                      underlyingTokenDecimals[index]
                    )
                  );
                  underlyingTokensTvlUsd +=
                    underlyingTokenBalance * underlyingTokenPrice;
                }
                stakingTokenPrice =
                  underlyingTokensTvlUsd /
                  Number(
                    formatUnits(stakingTokenTotalSupply, stakingTokenDecimals)
                  );
              }
      ...
      for (const {
        ...
        output: { rewardRate, periodFinish },
      } of rewardDataResults) {
        ...
        const rewardsPerYearRaw =
          BigNumber.from(rewardRate).mul(SECONDS_PER_YEAR);
      ...
      return Object.entries(chainFarms)
        .map(([farmAddress, { stakingTokenPool, pool }]) => {
          const tvlUsd = tvlUsdChainFarms[farmAddress];
          if (tvlUsd === undefined) {
            return undefined;
          }
          const stakingTokenYieldPool = yieldPools.find(
            ({ pool }) => pool === stakingTokenPool
          );
          return {
