    ...
    Object.entries(FARMS).map(async ([chain, chainFarms]) => {
      ...
      const [
        ...
        sdk.api.abi
          ...
          .then((call) =>
            call.output.map((x) => [x.input.target.toLowerCase(), x])
          )
          .then(Object.fromEntries),
        utils.getPrices(
          chainFarms.flatMap(({ rewardTokens }) => rewardTokens),
          chain
        ),
        sdk.api.abi
          .multiCall({
      ...
      const tvlUsdChainFarms = Object.fromEntries(
        chainFarms.map(({ address }) => {
          const totalSupply = formatUnits(
            totalSupplyResults[address.toLowerCase()].output,
            18
          );
      ...
      const aprs = Object.fromEntries(
        chainFarms.map(({ address }) => [address.toLowerCase(), 0])
      );
      rewardDataResults.forEach(
        ({
          input: {
            target: farm,
            params: [rewardToken],
          ...
          const rewardsPerYearUsd =
            rewardsPerYear * prices.pricesByAddress[rewardToken.toLowerCase()];
          const rewardApr =
            (rewardsPerYearUsd / tvlUsdChainFarms[farm.toLowerCase()]) * 100;

          aprs[farm.toLowerCase()] = aprs[farm.toLowerCase()] + rewardApr;
        }
      );
      return chainFarms.map(
        ({
          address,
          rewardTokens,
          stakingToken,
          ...
          return {
            ...
            symbol:
              symbol ??
              utils.formatSymbol(symbols[stakingToken.toLowerCase()].output),
