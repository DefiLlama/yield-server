...
const ChainConfig = {
  ...
  optimism: {
    ...
    chainTokens: ["0x4200000000000000000000000000000000000042", "0x39FdE572a18448F8139b7788099F0a0740f51205"], // OP, OATH
  },
  xdai: {
    booster: "0x98Ef32edd24e2c92525E59afc4475C1242a30184",
    balancerVault: "0xBA12222222228d8Ba445958a75a0704d566BF2C8",
...
const main = async () => {
  ...
  const rewardAssetKeys = [RewardAssetConfig.auraAddress, RewardAssetConfig.balAddress]
    .map((i) => `ethereum:${i}`)
    .join(',')
    .toLowerCase();
  const rewardTokenPrices = (await utils.getData(`https://coins.llama.fi/prices/current/${rewardAssetKeys}`)).coins;
  const auraPrice = rewardTokenPrices[`ethereum:${RewardAssetConfig.auraAddress.toLowerCase()}`].price
  const balPrice = rewardTokenPrices[`ethereum:${RewardAssetConfig.balAddress.toLowerCase()}`].price
  const allPools = []
  for (let chain in ChainConfig) {
    ...
    const allTokenKeys = [...lpTokens, auraAddress, balAddress, ldoAddress, ...(chainTokens !== undefined ? chainTokens : [])]
      .map((i) => `${chain}:${i}`)
      .join(',')
      .toLowerCase();
    ...
    const auraAPYs = _.range(validPoolsLength).map(
      (i) =>
        ((auraRewardPerYearRates[i] / 1e18) * auraPrice * 100) /
        poolTVLs[i]
    );
    ...
    const chainPools = await Promise.all(
      _.range(validPoolsLength).map(async (i) => {
        ...
        for (let x = 0; x < extraRewardLengths[i]; x++) {
          ...
          } else if ((chain === "ethereum" && ![ldoAddress, auraAddress].includes(baseToken)) || !(`${chain}:${baseToken.toLowerCase()}` in tokenPrices)) {
            console.log(validPoolIds[i], "new reward token. please add support for", baseToken)
            const newTokenPrices = (
              await utils.getData(`https://coins.llama.fi/prices/current/${chain}:${baseToken.toLowerCase()}`)
            ).coins;
            tokenPrices[`${chain}:${baseToken.toLowerCase()}`] = newTokenPrices[`${chain}:${baseToken.toLowerCase()}`]
            tokenPrice = newTokenPrices[`${chain}:${baseToken.toLowerCase()}`].price
          } else {
            tokenPrice = tokenPrices[`${chain}:${baseToken.toLowerCase()}`].price
          }
