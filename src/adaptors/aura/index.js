...
const main = async () => {
  ...
  const auraAPYs = _.range(validPoolsLength).map(
    (i) =>
      ((auraRewardPerYearRates[i] / 1e18) *
        tokenPrices[`${chain}:${AURA_ADDRESS.toLowerCase()}`].price *
        100) /
      poolTVLs[i]
  );
  ...
  const finalPools = await Promise.all(
    _.range(validPoolsLength).map(async (i) => {
      const data = {
        ...
        rewardTokens: [
          ethers.utils.getAddress(AURA_ADDRESS),
          ethers.utils.getAddress(BAL_ADDRESS),
        ],
  ...
  return finalPools.filter((p) => utils.keepFinite(p));
};
