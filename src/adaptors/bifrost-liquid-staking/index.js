...
const getApy = async () => {
  const contract_veth =
    (await sdk.api.erc20.totalSupply({ target: veth })).output / 1e18;
  const contract_veth1 =
    (await sdk.api.erc20.totalSupply({ target: veth_1 })).output / 1e18;
  const contract_veth1_null_address_balance =
    (
  ...
  const vETH = {
    ...
    symbol: 'veth',
    tvlUsd:
      new BigNumber(contract_veth)
        .plus(contract_veth1)
        .minus(contract_veth1_null_address_balance)
        .toNumber() * prices['coingecko:ethereum'].price,
