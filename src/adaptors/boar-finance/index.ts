 ...
 *   2. Resolve epoch-start block from the event timestamp via sdk.api.util.lookupBlock
...
const getApy = async (): Promise<Pool[]> => {
  ...
  `;
	...
	const { block: epochStartBlock } = await sdk.api.util.lookupBlock(
		epochStart,
		{ chain: CHAIN },
	);
	...
	const [lockedRes, currentLockedRes, yieldRes, priceData] = await Promise.all([
		// epoch-start locked amount → APY denominator
		sdk.api.abi.call({
			target: VE_BTC,
			abi: LOCKED_ABI,
