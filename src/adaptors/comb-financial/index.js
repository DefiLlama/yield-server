...
function getSymbol(poolId) {
  mappings.find(pool => pool.pool === poolId) ? symbol = mappings.find(pool => pool.pool === poolId).symbol : symbol = null;

  return symbol;
}


function getTokens(poolId) {
  mappings.find(pool => pool.pool === poolId) ? tokens = mappings.find(pool => pool.pool === poolId).tokens : tokens = null;
