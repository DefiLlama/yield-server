...
async function getTvl(pair, liquidity, baseTokenPrice, tokenB, chainString) {
  const totalSupply = (
    await sdk.api.erc20.totalSupply({ target: pair, chain: chainString })
  ).output;
  const balance = (
  ...
  const baseTokenAmount = BigNumber(liquidity)
    .times(balance)
    .div(totalSupply)
  ...
  for (const item of Object.values(data.Farms)) {
    ...
    const farmLiquidity = (
      await sdk.api.abi.multiCall({
        abi: balanceOfAbi,
        calls: item.map((a) => {
          return { target: a.pairAddress, params: a.farmAddress };
        }),
        chain: item[0].chainString,
      })
    ).output;

    const baseTokenPrices = await utils.getPrices(
      item.map((a) => {
        return a.tokenB.address;
      }),
      item[0].chainString
    );
    ...
    for (let i = 0; i < farmLiquidity.length; i++) {
      const promise = getTvl(
        item[i].pairAddress,
        farmLiquidity[i].output,
        baseTokenPrices.pricesByAddress[item[i].tokenB.address.toLowerCase()],
        item[i].tokenB,
        item[0].chainString
    ...
    await Promise.all(promises).then((results) => {
      poolList = results.map((item) => {
        return { tvlUsd: item };
      });
    });

    const rewardTokenPrices = await utils.getPrices(
      item.map((a) => {
        return a.rewardToken.address;
      }),
      item[0].chainString
    );
    ...
    for (let i = 0; i < rewardRate.length; i++) {
      poolList[i]['apyReward'] = getApy(
        rewardRate[i].output,
        rewardTokenPrices.pricesByAddress[
          item[i].rewardToken.address.toLowerCase()
        ],
        poolList[i].tvlUsd,
        item[i].multiplier
      );
