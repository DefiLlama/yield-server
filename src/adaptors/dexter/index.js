...
const query = gql`{
    atom_price_data: price_data(limit: 1, where: {asset_id: {_eq: "cosmos"}}, order_by: {timestamp: desc}) {
        price_usd
    }
    stkatom_price_data: price_data(limit: 1, where: {asset_id: {_eq: "stkatom"}}, order_by: {timestamp: desc}) {
        price_usd
    }
    pools {
        poolId
...
async function apy() {
    const data = await request('https://api.core-1.dexter.zone/v1/graphql', query);
    const res = await superagent.get('https://api.persistence.one/pstake/stkatom/apr');
    ...
    data.pools.forEach((p) => {
        ...
        if (p.poolId === 1) {
            const atomsUsd = new BigNumber(p.poolAssets[0].amount).times(data.atom_price_data[0].price_usd);
            const stkAtomsUsd = new BigNumber(p.poolAssets[1].amount).times(data.stkatom_price_data[0].price_usd);
