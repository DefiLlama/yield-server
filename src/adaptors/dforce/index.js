...
const getStakingApy = async () => {
  const stakingDetails = await Promise.all(
    Object.keys(STAKING_CONTRACTS).map(async (network) => {
      return {
        network,
        apyData: await utils.getData(
          getStakingApiUrl(STAKING_CONTRACTS[network].name)
        ),
  ...
  let stakingData = [];
  stakingDetails.map(async ({ network, apyData, poolData }) => {
...
const getVaultApy = async () => {
  ...
  const vaultData = await Promise.all(
    vaultDetails.map(async ({ network, poolData }) => {
      return await Promise.all(
        poolData.vaultPools.map(async (vaultPool) => {
          ...
          let rawTvlUsd =
            Number(poolInfoResult._supplyValue) -
            Number(poolInfoResult._borrowValue);
