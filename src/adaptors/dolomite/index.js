...
async function apy() {
  return Object.keys(DOLOMITE_MARGIN_ADDRESS_MAP).reduce(async (memo, chain) => {
    ...
    const marginPremiums = marginPremiumsRes.output.map((o) => Number(o.output) / 1e18);
    ...
    const supplyWeis = totalPars.map((totalPar, i) => Number(totalPar.supply) * Number(indices[i].supply) / 1e18);
    const borrowWeis = totalPars.map((totalPar, i) => Number(totalPar.borrow) * Number(indices[i].borrow) / 1e18);
    ...
    const supplyInterestRateApys = borrowInterestRateApys.map((interestRate, i) => {
      if (interestRate === 0) {
        ...
        return interestRate * earningsRate * borrowWeis[i] / supplyWeis[i];
      }
    });
