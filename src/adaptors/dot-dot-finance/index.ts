...
const getApy = async () => {
  // Return empty array if API is unavailable (protocol deprecated)
  try {
    const {
      data: { tokens },
    ...
    // API is down (402 Payment Required) - protocol is deprecated
    console.log('dot-dot-finance: API unavailable, returning empty array');
    return [];
  }
};
