...
const gvEase = async () => {
  ...
  const rateExpireAmtHex = web3.utils.padLeft('0x' + bribeRates.slice(34,67), 64);
  const rateExpireAmt = web3.eth.abi.decodeParameter('uint256', rateExpireAmtHex);
  ...
  const easePrice = (
    await superagent.post('https://coins.llama.fi/prices').send({
      coins: ["coingecko:ease"]
    })
  ).body.coins['coingecko:ease'].price;
