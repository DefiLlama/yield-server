...
const reward_tracker_arbitrum_elp1 =
  '0x2108397905f6d3a9b277c545948c6d6e1ca22d06';
const reward_router_arbitrum_elp1 =
  '0x86af1e551c081ec2269f62708c291af1627fa4ed';
...
async function getVaultTvl(vault, vault_tokens, pChain, priceDataRes) {
  ...
  for (let i = 0; i < vault_tokens.length; i++) {
    ...
    tvl =
      tvl +
      (priceDataRes[token_map_arbirtrum_elp1[token]] / 10 ** 30) *
        (vault_balance.output / 10 ** decimals.output);
  }
  // console.log("tvl::", tvl)
...
async function getVaultTvl_bsc(vault, vault_tokens, pChain, priceDataRes) {
  ...
  for (let i = 0; i < vault_tokens.length; i++) {
    ...
    tvl =
      tvl +
      (priceDataRes[token_map_bsc_elp1[token]] / 10 ** 30) *
        (vault_balance.output / 10 ** decimals.output);
  }
  // console.log("tvl::", tvl)
...
const getPools = async () => {
  let edePrice = (await utils.getData('https://data.ede.finance/arb/edekline'))
    .data[0].price;
  let elpPrice = 1;
  ...
  const totalStaked_elp1 = Number(
    ethers.utils.formatUnits(arb_info.output[1][0], 18)
  );
  const stakingPool_elp1 = Number(
    ethers.utils.formatUnits(arb_info.output[2][0], 18)
  );
  // console.log("totalStaked_elp1::", totalStaked_elp1)
  // console.log("stakingPool_elp1::", stakingPool_elp1)

  const apr_ede_elp =
    (stakingPool_elp1 * 3600 * 24 * 365 * edePrice) /
    (totalStaked_elp1 * elpPrice);
  ...
  const elpManager_totalSupply = Number(
    ethers.utils.formatUnits(elpManager_info.output[2], 18)
  );

  // console.log("elpManager_info::", elpManager_info.output);

  const apr_eusd_elp =
    (((Number(ethers.utils.formatUnits(feeAmount_arb.output, 18)) / 1) * 365) /
      (1 * elpManager_totalSupply * elpPrice)) *
    0.6;
  ...
  const priceDataRes = await utils.getData('https://api.ede.finance/prices');

  // console.log("priceDataRes::", priceDataRes)

  let tvl_arbitrum_elp1 = await getVaultTvl(
    vault_arbitrum_elp1,
    vault_tokens_arbitrum_elp1,
    'arbitrum',
    priceDataRes
  );
  ...
  const totalStaked_elp1_bsc = Number(
    ethers.utils.formatUnits(bsc_info.output[1][2], 18)
  );
  const stakingPool_elp1_bsc = Number(
    ethers.utils.formatUnits(bsc_info.output[2][2], 18)
  );
  console.log('totalStaked_elp1_bsc::', totalStaked_elp1_bsc);
  console.log('stakingPool_elp1_bsc::', stakingPool_elp1_bsc);

  edePrice = (await utils.getData('https://data.ede.finance/bsc/edekline'))
    .data[0].price;

  elpPrice = 0.92;
  const apr_ede_elp_bsc =
    (stakingPool_elp1_bsc * 3600 * 24 * 365 * edePrice) /
    (totalStaked_elp1_bsc * elpPrice);
  ...
  const elpManager_totalSupply_bsc = Number(
    ethers.utils.formatUnits(elpManager_info_bsc.output[2], 18)
  );

  console.log('elpManager_totalSupply_bsc::', elpManager_totalSupply_bsc);

  const apr_eusd_elp_bsc =
    (((Number(ethers.utils.formatUnits(feeAmount_bsc, 18)) / 1) * 365) /
      (1 * elpManager_totalSupply_bsc * elpPrice)) *
    0.6;

  console.log('apr_eusd_elp_bsc::', apr_eusd_elp_bsc);

  let tvl_bsc_elp1 = await getVaultTvl_bsc(
    vault_bsc_elp1,
    vault_tokens_bsc_elp1,
    'bsc',
    priceDataRes
  );
