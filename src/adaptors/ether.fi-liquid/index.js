...
const getLiquidHYPEOracleData = async () => {
  ...
  try {
    ...
    for (let roundId = currentRoundId - 1; roundId >= 1 && roundId >= currentRoundId - 50; roundId--) {
      try {
...
const apy = async () => {
  ...
  const [liquidETHRates, liquidUSDRates, liquidBTCRates, liquidHYPEOracleData] =
    await Promise.all([
      Promise.all([
  ...
  // Get total supplies for Ethereum vaults
  const [liquidETHSupply, liquidUSDSupply, liquidBTCSupply] = await Promise.all(
    [
      sdk.api.abi.call({ target: liquidETH, abi: 'erc20:totalSupply' }),
      sdk.api.abi.call({ target: liquidUSD, abi: 'erc20:totalSupply' }),
      sdk.api.abi.call({ target: liquidBTC, abi: 'erc20:totalSupply' }),
    ]
  );

  // Get total supply for HYPE vault on Hyperliquid
  const liquidHYPESupply = await sdk.api.abi.call({
    target: liquidHYPE,
  ...
  const weETHPrice = pricesRes.data.coins[`ethereum:${weETH}`]?.price || 0;
  const usdcPrice = pricesRes.data.coins[`ethereum:${USDC}`]?.price || 1;
  ...
  // Calculate liquidETH APY (rate is in 1e18, represents value per share in weETH terms)
  ...
  const liquidETHApr7d =
    liquidETHRate7dAgo > 0
      ? ((liquidETHRateCurrent - liquidETHRate7dAgo) / liquidETHRate7dAgo / 7) *
        365 *
        100
      : 0;

  // Calculate liquidUSD APY (rate is in 1e6, represents value per share in USDC terms)
  ...
  const liquidUSDApr7d =
    liquidUSDRate7dAgo > 0
      ? ((liquidUSDRateCurrent - liquidUSDRate7dAgo) / liquidUSDRate7dAgo / 7) *
        365 *
        100
      : 0;

  // Calculate liquidBTC APY (rate is in 1e8, represents value per share in WBTC terms)
  ...
  const liquidBTCApr7d =
    liquidBTCRate7dAgo > 0
      ? ((liquidBTCRateCurrent - liquidBTCRate7dAgo) / liquidBTCRate7dAgo / 7) *
        365 *
        100
      : 0;
  ...
  if (liquidHYPEOracleData) {
    const { currentRate, currentTimestamp, rate1dAgo, timestamp1dAgo, rate7dAgo, timestamp7dAgo } =
  ...
  // Calculate TVL
  // liquidETH: totalSupply * rate * weETH price (rate is value per share in weETH)
  const liquidETHTvl =
    (Number(liquidETHSupply.output) / 1e18) *
    (liquidETHRateCurrent / 1e18) *
    weETHPrice;

  // liquidUSD: totalSupply * rate * usdc price
  // Both token and rate are scaled to 1e6 (USDC decimals)
  const liquidUSDTvl =
    (Number(liquidUSDSupply.output) / 1e6) *
    (liquidUSDRateCurrent / 1e6) *
    usdcPrice;

  // liquidBTC: totalSupply * rate * WBTC price
  // Both token and rate are scaled to 1e8 (BTC decimals)
  const liquidBTCTvl =
    (Number(liquidBTCSupply.output) / 1e8) *
    (liquidBTCRateCurrent / 1e8) *
    wbtcPrice;

  // liquidHYPE: totalSupply * rate * HYPE price
  const liquidHYPERate = liquidHYPEOracleData?.currentRate || 1;
  const liquidHYPETvl =
    (Number(liquidHYPESupply.output) / 1e18) * liquidHYPERate * hypePrice;
