...
const primaryContractAddys = [
  {
    chain: 'ethereum',
    chainSpecificProtocolContractAddy:
      '0x46558DA82Be1ae1955DE6d6146F8D2c1FE2f9C5E',
    blocksPerDay: 7200,
  },
];
...
const allLendingTokens = async () => {
  ...
  for (let i = 0; i < primaryContractAddys.length; i++) {
    ...
    for (let i = 0; i < lendingTokensCount; i++) {
      ...
      let lendingTokenPoolAddy =
        lendingTokenPoolAddyRes['output']['bLendingToken'].toLowerCase();
      ...
      let lenderAPY = await getLenderAPY(
        chainSpecific.chain,
        lendingTokenPoolAddy,
        chainSpecific.blocksPerDay
      );
      let borrowAPY = await getBorrowAPY(
        chainSpecific.chain,
        lendingTokenPoolAddy,
        chainSpecific.blocksPerDay
      );
      ...
      let balanceOfLendingToken =
        balanceOfLendingTokenRes['output'] / 10 ** decimalsOfLendingToken;

      // Get conversion factor to USD.
      let priceUSDRes = await getPrices(chainSpecific.chain, [
        underlyingTokenAddy,
      ]);
