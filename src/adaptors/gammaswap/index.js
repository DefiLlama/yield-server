...
function supplyApy(snapshot, poolInfo) {
  const avgDecimals =
    (Number(poolInfo.decimals[0]) + Number(poolInfo.decimals[1])) / 2;
  const totalLiquidityBefore = Number(
    etherUtils.formatUnits(snapshot.totalLiquidity, avgDecimals)
  );
  const totalLiquidityAfter = Number(
    etherUtils.formatUnits(
      BigNumber.from(poolInfo.BORROWED_INVARIANT).add(
        BigNumber.from(poolInfo.LP_INVARIANT)
      ),
      avgDecimals
    )
  );
  ...
  const totalSupplyBefore = Number(
    etherUtils.formatUnits(snapshot.totalSupply, 18)
  );
  const totalSupplyAfter = Number(
    etherUtils.formatUnits(poolInfo.totalSupply, 18)
  );
...
function borrowApy(snapshot, poolInfo) {
  const accFeeIndex1DayAgoNum = Number(
    etherUtils.formatUnits(snapshot.accFeeIndex, 18)
  );
  const accFeeIndexNum = Number(
    etherUtils.formatUnits(poolInfo.accFeeIndex, 18)
  );
...
async function apy() {
  const chainInfo = [
    {
      chainName: 'arbitrum',
      endpoint:
        'https://api.thegraph.com/subgraphs/name/gammaswap/gammaswap-v1-arbitrum',
      refPoolAddr: '0x63c531ffed7e17f8adca4ed490837838f6fa1b66',
    },
    {
      chainName: 'base',
      endpoint:
        'https://api.studio.thegraph.com/query/49518/gammaswap-v1-base/version/latest',
  ...
  for (let i = 0; i < chainInfo.length; i++) {
    const _pools = await apyPerChain(
      chainInfo[i].chainName,
      chainInfo[i].refPoolAddr,
      chainInfo[i].endpoint
    );
...
async function apyPerChain(chainName, refPoolAddr, endpoint) {
  ...
  let _latestPoolsData = latestPoolsData.filter(function (pool) {
    return (
      (Number(pool.output.decimals[0]) + Number(pool.output.decimals[1])) % 2 ==
      0
    );
  });
  ...
  const _pools = pools.filter(function (pool, i) {
    return (
      (Number(latestPoolsData[i].output.decimals[0]) +
        Number(latestPoolsData[i].output.decimals[1])) %
        2 ==
      0
    );
  });
  ...
  return _pools.map((pool, i) => ({
    ...
    project: 'gammaswap',
    symbol: formatSymbols(
      chainName,
      _latestPoolsData[i].output.symbols,
      _latestPoolsData[i].output.tokens
    ),
