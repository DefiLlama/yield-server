...
const poolInfo = async (chain) => {
  ...
  const gearPerBlock = {
    ...
    '0xB03670c20F87f2169A7c4eBE35746007e9575901': { LP: 31.96, CA: 2.30 },
  ...
  const dTokenDecimals = decimals.map((decimal) => Math.pow(10, Number(decimal)));
...
function calculateTvl(availableLiquidity, totalBorrowed, price, decimals) {
  ...
  const tvl =
    (parseFloat(availableLiquidity) + parseFloat(totalBorrowed)) /
    decimals *
    price;
  return tvl;
};
...
const getApy = async () => {
  ...
  const pools = (await poolInfo('ethereum')).yieldPools.map(
    (pool, i) => {
