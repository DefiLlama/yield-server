...
const getPoolsChain = async (chain, eventEmitter, fromBlock) => {
    ...
    const pools = await Promise.all(logs.map(async i => {
        const [market, index, long, short] = i[4].addressItems.items.map(i => i.value)
        const symbol = (
            await sdk.api.abi.multiCall({
                abi: 'erc20:symbol',
                calls: [long, short].map((p) => ({ target: p })),
                chain: 'ethereum',
            })
        ).output.map((o) => o.output);

        // tvl = calculate long and short tokens held in market address
