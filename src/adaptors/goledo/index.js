...
const getBaseTokensPrice = async () => {
  const golUsd = (await utils.getData(
    'https://api.coingecko.com/api/v3/simple/price?ids=goledo&vs_currencies=usd'
  ));
  
  const cfxUsd = (await utils.getData(
    'https://api.coingecko.com/api/v3/simple/price?ids=conflux-token&vs_currencies=usd'
  ));
  

  const cfxPrice = cfxUsd['conflux-token'].usd;
  const golPrice = golUsd['goledo'].usd;
...
const calculateReservesUSD = (
  ...
  const reserve0 = new BigNumber(reserves[0]._reserve0)
    .times(reservesRatio);
  const reserve1 = new BigNumber(reserves[0]._reserve1)
    .times(reservesRatio);
...
const getApy = async () => {
  ...
  const normalizedGOLPerBlock = GOLPerSecond.output / 1e18 * CONFLUX_BLOCK_TIME;
