...
async function apy() {
  ...
  const [hiveBalancesGrizzly,
    ...
    yearnBalances] = await Promise.all([
      sdk.api.abi.multiCall({ calls: pcsHives.map(getHive), abi: abiGrizzly, chain: 'bsc', }),
      sdk.api.abi.multiCall({ calls: pcsHives.map(getHive), abi: abiStandard, chain: 'bsc', }),
      sdk.api.abi.multiCall({ calls: pcsHives.map(getHive), abi: abiStable, chain: 'bsc', }),
      sdk.api.abi.multiCall({ calls: stableHives.map(getHive), abi: abiGrizzly, chain: 'bsc', }),
      sdk.api.abi.multiCall({ calls: stableHives.map(getHive), abi: abiStandard, chain: 'bsc', }),
      sdk.api.abi.multiCall({ calls: stableHives.map(getHive), abi: abiStable, chain: 'bsc', }),
      sdk.api.abi.multiCall({ calls: farms.map(getHive), abi: abiFarm, chain: 'bsc', }),
  ...
  hiveBalances.push(...hiveBalancesGrizzly.output.map((grizzly, i) => {
  ...
  const res = pcsHives.concat(farms).concat(yearnHives).concat(stableHives).map((pool, i) => {
    ...
    const fields = apyComputed
      .find(e => e.point.toLowerCase() === pool.hive.toLowerCase())
      .fields;
