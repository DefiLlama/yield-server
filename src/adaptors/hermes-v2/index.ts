...
const getAllGauges = async (currentBlock) => {
  const logs = await sdk.getEventLogs({
    ...
    eventAbi: 'event AddGauge(address indexed gauge)',
...
const getStakedTokenIds = async (
  ...
  try {
    // Query TokenStaked events (all 3 params are indexed)
    const logs = await sdk.getEventLogs({
      target: ADDRESSES.UNISWAP_V3_STAKER,
      eventAbi:
        'event TokenStaked(uint256 indexed tokenId, bytes32 indexed incentiveId, bool indexed isRestake)',
    ...
    // Filter for matching incentiveId and deduplicate
    const normalizedId = incentiveId.toLowerCase();
    const tokenIds = logs
      .filter((log: any) => log.args.incentiveId?.toLowerCase() === normalizedId)
      .map((log: any) => BigInt(log.args.tokenId));
