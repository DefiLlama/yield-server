...
const getData = async (target, block, hydtPrice) => {
    ...
    const hygtPrice = (parseFloat(hydtPrice) * hygtPairInfo[0].reserve1) / parseFloat(hygtPairInfo[0].reserve0);
...
const getEarnAPY = async (block, blockPrior, blockPrior7d, hydtPrice) => {
    const [
        ...
        hygtPer,
        hygtPrice
    ] = await getData(earn, block, hydtPrice);
    ...
    // calculate apy
    data = data.map((i) =>
        utils.apy(i, dataPrior, dataPrior7d, 'v2')
    );
    ...
    data.forEach((p) => {
        const totalDeposit = new BigNumber(p.totalDeposit).dividedBy(new BigNumber(10).pow(18)).toFixed(18);
...
const getFarmAPY = async (block, blockPrior, blockPrior7d, hydtPrice) => {
    const [
        ...
        hygtPer,
        hygtPrice
    ] = await getData(farm, block, hydtPrice);
    ...
    data = data.map(i => {
        const price0 = i.token0.id == hydt ? hydtPrice : i.token0.id == hygt ? hygtPrice : i.price0;
        const price1 = i.token1.id == hydt ? hydtPrice : i.token1.id == hygt ? hygtPrice : i.price1;
    ...
    data = data.map((p) => {
        ...
        const totalDeposit = new BigNumber(p.totalDeposit).dividedBy(new BigNumber(10).pow(18)).toFixed(18);
        const totalSupply = new BigNumber(p.totalSupply).dividedBy(new BigNumber(10).pow(18)).toFixed(18);
