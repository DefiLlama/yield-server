...
const calculateAPR = async (config, currentRate) => {
    ...
    const block1dayAgoCall = axios.get(`https://coins.llama.fi/block/${chain}/${timestamp1dayAgo}`);
    const block7dayAgoCall = axios.get(`https://coins.llama.fi/block/${chain}/${timestamp7dayAgo}`);
 ...
 */
const apy = async () => {
    const out = await Promise.all(config.map(async (chainConfig) => {
