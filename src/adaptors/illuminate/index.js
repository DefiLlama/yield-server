...
async function getTvlPool(pt, pool) {
    ...
    const virtualFyTokenBalance = (await sdk.api.abi.call({
        target: pool,
        abi: poolAbi.find((i) => i.name === 'getFYTokenBalance'),
        chain: 'ethereum',
    })).output;
    const poolTotalSupply = (await sdk.api.abi.call({
        target: pool,
        abi: poolAbi.find((i) => i.name === 'totalSupply'),
        chain: 'ethereum',
    })).output;
    const fyTokenTvl = (((virtualFyTokenBalance - poolTotalSupply) * fyTokenValue) / one) / one;
    // Get the amount of underlying held by the pool (value of shares token)
    const baseTokenTvl = (await sdk.api.abi.call({
        target: pool,
