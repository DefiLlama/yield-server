...
const poolsFunction = async() => {
    ...
    await Promise.all(Object.values(networks).map(async(value, key) => {
        let tokenNames = Object.keys(tokensList[key]);
        let tokenAddresses = Object.values(tokensList[key]);
        await Promise.all(tokenAddresses.map(async(tokenAddress, nameIndex) => {
            let price = await getPrice(value, tokenAddress);
            let cTokenPrice = await getCropPrice(value, tokenNames[nameIndex].replace("x", ""));
            ...
            let pool = {
                ...
                tvlUsd: locked * price,
                apy: apy,
                poolMeta: "APY doesn't include inflation protection. The protocol determines it as near 10%"
            }
            pools.push(pool);
        }));
    }));
