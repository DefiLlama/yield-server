...
const getLpsTvl = async (chain, factory, lps) => {
  ...
  const pairAddresses = resultPairs.output.map((pairResult) => (pairResult.output))
  ...
  const reserves = resultReserves.output.map((pairResult) => ({
    ...
    lp: lps.find((lp) => lp.lp_address === pairResult.input.target && lp.chain_id === chain.chainId)
  }));
  ...
  return lps.map((lp) => ({
    ...
    totalValueLockedUSD: Number(poolsTvl.find((item) => item.lpAddress === lp.lp_address)?.totalValueLockedUSD)
  }));
}
  ...
  if (data !== undefined) {
    ...
    for (const chain of CHAINS) {
      ...
      poolsApy.forEach(function(pool) {
        ...
        pool.tvlUsd = tvl !== undefined && isFinite(tvl.totalValueLockedUSD) ? tvl.totalValueLockedUSD : pool.tvlUsd
      });
    }
  }
