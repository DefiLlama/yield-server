...
function decodeCandid(bytes, labelHashMap) {
  ...
  const decodeType = (typeRef) => {
    ...
    if (typeRef === CANDID_TYPE.nat8) return Number(decodeFixedNat(bytes, state, 1))
    if (typeRef === CANDID_TYPE.nat16) return Number(decodeFixedNat(bytes, state, 2))
    if (typeRef === CANDID_TYPE.nat32) return Number(decodeFixedNat(bytes, state, 4))
