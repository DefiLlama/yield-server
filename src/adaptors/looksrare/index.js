...
const poolInfo = async (chain) => {
  ...
  const aggregatorSharesValueInLooks = (
    await sdk.api.abi.call({
  ...
  const rewardPerBlockForStaking = (
    await sdk.api.abi.call({
      abi: abi.rewardPerBlockForStaking,
      target: distributor,
      chain: chain,
    })
  ).output;
...
function dailyWethCompounds(
  ...
  const aggregatorWethRewardsShare =
    aggregatorSharesValueInLooks / totalAmountStaked;
  const aggregatorWethRewardPerDay =
    (rewardPerBlock / 1e18) * aggregatorWethRewardsShare * BLOCKS;
...
const getApy = async () => {
  ...
  const standardTvl = calculateTvl(
    pool.standardReserve,
    prices[looksrare.toLowerCase()]
  );
  const compounderTvl = calculateTvl(
    pool.compounderReserve,
    prices[looksrare.toLowerCase()]
  );
  const dailyCompounds = dailyWethCompounds(
    pool.aggregatorSharesValueInLooks,
    pool.totalAmountStaked,
    pool.rewardPerBlock,
    pool.thresholdAmount
  );
  const wethApr = calculateApr(
    pool.rewardPerBlock,
    prices[weth.toLowerCase()],
    standardTvl + compounderTvl
  );
  const wethApy = calculateApy(wethApr, dailyCompounds);
  const looksApr = calculateApr(
    pool.rewardPerBlockForStaking,
    prices[looksrare.toLowerCase()],
    standardTvl + compounderTvl
  );
  ...
  pools.push(
    exportFormatter(
      ...
      chain,
      compounderTvl,
      compounderApy(wethApy, looksApr),
      null,
      [pool.rewardTokens[1]],
      'LOOKS Compounder'
    )
