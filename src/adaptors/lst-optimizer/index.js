...
const bigToNum = (bn) => {
  if (typeof bn == 'string') ethers.BigNumber.from(bn);
...
const queryStakingAprs = async () => {
  const rETHResult = (
    await axios.get(
      'https://dt4w6sqvtl.execute-api.eu-central-1.amazonaws.com/mainnet/'
    )
  ).data;
...
const topLvl = async (chainString, underlying) => {
  ...
  const stETHValue = bigToNum(
    kETHAssets.find(
      (item) => item.token.toLowerCase() == wstETHTokenAddress.toLowerCase()
    ).valueInETH
  );

  const rETHValue = bigToNum(
    kETHAssets.find(
      (item) => item.token.toLowerCase() == rETHTokenAddress.toLowerCase()
    ).valueInETH
  );
  ...
  const dETHValue =
    bigToNum(
      kETHAssets.find(
        (item) => item.token.toLowerCase() == dETHTokenAddress.toLowerCase()
      ).valueInETH
    ) +
    bigToNum(
      kETHAssets.find(
        (item) => item.token.toLowerCase() == savETHTokenAddress.toLowerCase()
      ).valueInETH
    ) +
    bigToNum(
      kETHAssets.find(
        (item) => item.token.toLowerCase() == gETHTokenAddress.toLowerCase()
      ).valueInETH
    );
