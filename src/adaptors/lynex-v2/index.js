...
const getApy = async () => {
  ...
  const pairAPIContract = new web3.eth.Contract(
    pairAPI,
    '0x6c84329CC8c37376eb32db50a17F3bFc917c3665'
  );
  ...
  const pools = poolInfos.map((pool) => {
    ...
    return {
      ...
      totalSupply: fromWei(Number(pool.total_supply), Number(pool.decimals)),
  ...
  const returnData = pools.map((pool, i) => {
    ...
    // Get TVL
    try {
      const asset0Price = !pool.token0Address
        ? 0
        : prices.get(pool.token0Address.toLowerCase());
      const asset1Price = !pool.token1Address
        ? 0
        : prices.get(pool.token1Address.toLowerCase());
      if (asset0Price && asset1Price && asset0Price > 0 && asset1Price > 0) {
        tvl = pool.token0Reserve
          .times(asset0Price || 0)
          .plus(pool.token1Reserve.times(asset1Price));
      ...
      const lpPrice = pool.totalSupply.isZero()
        ? new BigNumber(0)
