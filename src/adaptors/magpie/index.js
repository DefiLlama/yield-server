...
const READER_ADDRESS = "0x0E7EB5fb8d4e1e9C872ebF7072383cf97434D2e2";
const CHAIN = 'bsc';
...
async function apy() {
  const magpieInfo = await getMagpieInfo()
  const poolAprList = [];
  ...
  let poolInfos = [];
  // const newPools = magpieInfo.pools.filter(item => {
  ...
  for(let i = 0, l = magpieInfo.pools.length; i < l; i++) {
    ...
    const pool = {
      ...
      locker: rawInfo.poolType === "MAGPIE_VLMGP_POOL" ? rawInfo.helper: AddressZero ,
