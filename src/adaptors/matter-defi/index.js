...
const fetchAllPools = async () => {
  ...
  return spicyPools.map(pool => ({ contract: pool.contract, reserve: pool.reservextz, token0: pool.token0, token1: pool.token1 }));
}

const fetchSpicyTokens = async (agg) => {
  if (!_spiceTokens) _spiceTokens = axios(`${SPICY_URL}/TokenList?day_agg_start=${agg}`);
...
const fetchMatterFarms = async(pools, tokens) => {
  ...
  const output = farms.reduce((a, p) => {
    ...
    const findToken = tokens.find(token => token.tag === `${p.key.fa2_address}:${p.key.token_id}`);
    const findPool = pools.find(pool => pool.contract === `${p.key.fa2_address}`);
  ...
  return output.map(farm => ({ key: farm.key, 
...
const lpToTez = (farm) => {
  if(!farm.reserveXtz) {
    ...
    const tezPerLp = farm.reserveXtz.dividedBy(farm.supply.shiftedBy(-farm.decimals));

    return tezPerLp.multipliedBy(farm.staked.shiftedBy(-farm.decimals)).toFixed(0);
  }
}
...
const tokenToApr = (token) => {
  ...
  const apr = combined > 0 ? new BigNumber(rewardPerSec * matterPrice * 86400 * 365 / combined * 100) : new BigNumber(0);
...
const poolToApr = (pool) => {
  ...
  const stakePrice = pool.reserveXtz.dividedBy(pool.supply).shiftedBy(-pool.decimals);
  const combined = totalStaked.multipliedBy(stakePrice);
