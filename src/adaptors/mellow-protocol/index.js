...
const getPrices = async (addresses, timestamp) => {
  ...
  const prices = (await axios.get(request.substr(0, request.length - 1))).data
    .coins;
...
const getTvl = async (tvl, tokens, prices, decimals) => {
  ...
  for (let i = 0; i < tokens.length; ++i) {
    result +=
      (minTvl[i] * prices[String(tokens[i])]) / 10 ** decimals[tokens[i]];
  }
...
const poolsFunction = async () => {
  ...
  const abiB = [
    'event VaultRegistered(address indexed origin, address indexed sender, uint256 indexed nft, address vault, address owner)',
  ];

  const rewardMap = {};
  rewardMap['0x13c7bCc2126d6892eEFd489Ad215A1a09F36AA9f'] =
    '0xbfafc964361f78754f523343b09b3cb7bb73bdd6';
  rewardMap['0x6A2Dd3B817F0364e7603e781dDA9c62f62c440E1'] =
    '0x0fd566cda6d6a3ae1760e1eebd22ee400cc79655';
  rewardMap['0x8E024f875f6fDdf1471582bed8504F46CB64487E'] =
    '0x6955ab1adefa2e48f449b88183a2774a186b7e61';
  ...
  for (let chain in config) {
    ...
    const registered = await contract.queryFilter(
      contract.filters.VaultRegistered(),
    ...
    for (let i = 0; i < Array.from(allTokens.values()).length; ++i) {
      decimalsKTV[Array.from(allTokens.values())[i]] =
        decimals.output[i].output;
    }
    ...
    for (let i = 0; i < tokens.output.length; ++i) {
      ...
      const timePassedShareYear =
        (currentTimestamp - timestampStart) / (86400 * 365);

      const lpPriceUsdDoNothing =
        (await getTvl(oldTvls.output[0].output, tokensI, prices, decimalsKTV)) /
        oldSupply.output[0].output;
      const lpPriceNow = tvlUsd / supplys.output[i].output;

      const apy =
        100 *
        ((1 + (lpPriceNow - lpPriceUsdDoNothing) / lpPriceUsdDoNothing) **
          (1 / timePassedShareYear) -
          1);
      ...
      if (vaults[i] in rewardMap) {
        ...
        if (finish.output[0].output > currentTimestamp) {
          ...
          const yearlyRewardUsd =
            (((currentReward.output[0].output * (86400 * 365)) /
              currentDuration.output[0].output) *
              prices[String(rewardToken)]) /
            10 ** decimalsKTV[rewardToken];
