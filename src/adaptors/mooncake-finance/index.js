...
const API_URLS = {
  base: 'https://api.studio.thegraph.com/query/51428/mooncakefi-base/version/latest',
};
  ...
  const pools = data.map(([chain, markets], i) => {
    const chainPools = markets.map((pool, idx) => {
      ...
      return {
        ...
        url: `https://mooncake.fi/reserve-overview/?underlyingAsset=${pool.aToken.underlyingAssetAddress}&marketName=${chainUrlParam[chain]}&utm_source=defillama&utm_medium=listing&utm_campaign=external`,
