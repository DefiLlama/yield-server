      ...
      calls: [
        {
          target:
            network == 'moonbeam'
              ? MOONBEAM_VIEWS_CONTRACT
              : network == 'base'
              ? BASE_VIEWS_CONTRACT
              : OPTIMISM_VIEWS_CONTRACT,
          params: [],
        },
      ],
...
const getApy = async () => {
  ...
  if (moonbeam_markets && moonbeam_markets.length == 1) {
    ...
    moonbeamResults = moonbeam_markets_data
      ...
      .map((pool) => {
        ...
        const market_info = ponder_markets.find(
          (r) =>
            r.address.toLowerCase() == market.toLowerCase() && r.chainId == 1284
        );
        if (
          !market_info ||
          typeof market_info.underlyingTokenAddress !== 'string'
        )
          return null;

        const token_info = ponder_tokens.find(
          (r) =>
        ...
        if (
          !token_info ||
          token_info.decimals === undefined ||
          token_info.decimals === null
        )
          return null;

        const totalSupplyScaled = Number(totalSupply) / Math.pow(10, 8);
        const totalBorrowsScaled =
          Number(totalBorrows) / Math.pow(10, token_info.decimals);
        const exchangeRateScaled =
          Number(exchangeRate) / Math.pow(10, token_info.decimals + 10);
        const underlyingPriceScaled =
          Number(underlyingPrice) / Math.pow(10, 36 - token_info.decimals);

        const totalSupplyUsd =
          Number(totalSupplyScaled) *
          Number(exchangeRateScaled) *
        ...
        const collateralFactorScaled =
          Number(collateralFactor) / Math.pow(10, 18);
  ...
  moonbeamResults = moonbeamResults || [];
  ...
  const moonbeam_prices_id = Object.values(moonbeam_incentives)
    .flat()
    .reduce((prev, curr) => {
  ...
  if (base_markets && base_markets.length == 1) {
    ...
    baseResults = base_markets_data
      ...
      .map((pool) => {
        ...
        const market_info = ponder_markets.find(
          (r) =>
            r.address.toLowerCase() == market.toLowerCase() && r.chainId == 8453
        );
        if (
          !market_info ||
          typeof market_info.underlyingTokenAddress !== 'string'
  ...
  const mrd_markets = await multicallMRD(
    baseResults.map((r) => r.pool.split('-')[0]),
    'base'
  );
  ...
  if (optimism_markets && optimism_markets.length == 1) {
    ...
    optimismResults = optimism_markets_data
      ...
      .map((pool) => {
        ...
        const market_info = ponder_markets.find(
          (r) =>
            r.address.toLowerCase() == market.toLowerCase() && r.chainId == 10
        );
        if (
          !market_info ||
          typeof market_info.underlyingTokenAddress !== 'string'
  ...
  optimismResults = optimismResults || [];

  const optimism_mrd_markets = await multicallMRD(
    optimismResults.map((r) => r.pool.split('-')[0]),
    'optimism'
  );

  const optimism_prices_id = optimism_mrd_markets
    .flat()
    .reduce((prev, curr) => {
      const [
        _owner,
  ...
  const optimism_mrd_prices = await getPrices(
    Object.values(optimism_prices_id)
  );
