...
const apr = async () => {
  ...
  const denomToGeckoIdMap = (
    await axios.get(`https://api-insights.carbon.network/info/denom_gecko_map`)
  ).data.result.gecko;
  ...
  for (const asset of allAssets) {
    ...
    const cdpAssetBalance =
      cdpModAccBalances.find((o) => o.denom === `cibt/${asset.denom}`)
        ?.available ?? 0;
    const cdpTotalSupply =
      cdpTotalSupplies.find((o) => o.denom === `cibt/${asset.denom}`)?.amount ??
      0;

    const { total_principal, utilization_rate } = debtInfo;
    const totalSupplied = new BigNumber(total_principal ?? 0).plus(
      new BigNumber(assetBalance?.available ?? 0)
    );
    const totalSuppliedUSD = totalSupplied
      .multipliedBy(usdValue)
      .shiftedBy(-Number(tokenInfo.decimals))
      .toNumber();
    const totalBorrowedUSD = new BigNumber(total_principal)
      .multipliedBy(usdValue)
      .shiftedBy(-Number(tokenInfo.decimals))
      .toNumber();
    ...
    //  get reward apy
    const totalCdpTokenUSD = getTotalCdpSharesUSD(
      debtInfo,
      params,
      borrowAPY,
      tokenInfo.decimals,
      bnOrZero(cdpTotalSupply),
      cdpAssetBalance,
      usdValue
    );
    const {
      tokenAprArr: tokenSupplyRewardArr,
      overallRewardApr: overallSupplyRewardApr,
    } = await getTokenAprArr(
      rewardSchemes,
      usdValues,
      tokens,
      'lend',
    ...
    const { tokenAprArr, overallRewardApr } = await getTokenAprArr(
      rewardSchemes,
      usdValues,
      tokens,
      'borrow',
