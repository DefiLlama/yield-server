...
const levManagerAbi = [
  "function lendingPairs(address) view returns (address)"
];
...
async function batchedMultiCall({ calls, chain, abi, type = 'unknown' }) {
  ...
  if (!metrics.multicallByType[type]) metrics.multicallByType[type] = { calls: 0, batches: 0 };
...
async function getAssetPrices(uniqueAssets, chainConfig) {
  ...
  const pricesArray = assetAddrs.map(addr => `${chainName}:${addr.toLowerCase()}`);
  ...
  try {
    ...
    for (const key of Object.keys(data.coins)) {
      if (!data.coins[key].price) {
        console.log(`[WARNING] No price found in API response for asset ${key}, setting price to 0.`);
    ...
    for (const addr of assetAddrs) {
      ...
      console.log(`[WARNING] Failed to fetch price for asset ${addr} on ${chainConfig.name}, setting price to 0.`);
    }
  }
  return prices;
}
...
async function getPoolsMulticallData(validLendingPairs, prices, chainConfig) {
  ...
  async function getAllBatchOutputs(calls, abi, type) {
    return await batchedMultiCall({ calls, chain: chainConfig.name.toLowerCase(), abi, type });
  }

  const accountingCalls = poolAddresses.map(addr => ({ target: addr, abi: "function getPairAccounting() view returns (uint128,uint128,uint128,uint128,uint256)" }));
  const nameCalls = poolAddresses.map(addr => ({ target: addr, abi: "function name() view returns (string)" }));
  const symbolCalls = poolAddresses.map(addr => ({ target: addr, abi: "function symbol() view returns (string)" }));
  const rateInfoCalls = poolAddresses.map(addr => ({ target: addr, abi: "function currentRateInfo() view returns (tuple(uint32 lastBlock, uint32 feeToProtocolRate, uint64 lastTimestamp, uint64 ratePerSec, uint64 fullUtilizationRate))" }));
  ...
  return { accountingRes: { output: accountingRes }, nameRes: { output: nameRes }, symbolRes: { output: symbolRes }, rateInfoRes: { output: rateInfoRes } };
}
...
async function main() {
  ...
  for (const [chainKey, chainConfig] of Object.entries(CHAINS)) {
    ...
    console.log("Discovered lending pairs for chain", chainConfig.name, ":", lendingPairs.map(x => x.lendingPair));
    ...
    const validLendingPairs = lendingPairs.filter(lp => lp.assetAddr && lp.assetDecimals);
    const { accountingRes, nameRes, symbolRes, rateInfoRes } = await getPoolsMulticallData(validLendingPairs, prices, chainConfig);
    ...
    const poolData = validLendingPairs.map((lp, i) => {
      ...
      if (assetPrice === undefined) {
        console.log(`[WARNING] No price found for asset in pool ${lp.lendingPair}. Setting assetPrice = 0.`);
        assetPrice = 0;
      }
      ...
      const protocolFee = Number(rateInfo.feeToProtocolRate ?? rateInfo[1] ?? 0) / 1e5;
      const borrowApr = Number(ratePerSec.toString()) * 60 * 60 * 24 * 365 / 1e18;
      const totalBorrowToken = Number((+totalBorrowAmount) / Math.pow(10, assetDecimals));
      const totalAssetToken = Number((+totalAssetAmount) / Math.pow(10, assetDecimals));
      const utilization = totalAssetToken > 0 ? totalBorrowToken / totalAssetToken : 0;
      ...
      const supplierApy = (Math.pow(1 + supplierApr / compoundPeriods, compoundPeriods) - 1);
...
// Print multicall usage metrics after completion
  ...
  console.log("Breakdown by type:", JSON.stringify(metrics.multicallByType, null, 2));
