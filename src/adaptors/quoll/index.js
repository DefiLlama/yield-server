...
Object.entries(abi).forEach(([contract, methods]) => {
  ...
  abi[contract] = Object.fromEntries(
...
BigNumber.prototype.toDecimal = function (option) {
  ...
  const fixedNumber = FixedNumber.fromValue(this, decimals, {
    ...DefaultFixedFormat,
    decimals,
  });
  const fixedStr =
    typeof precision === 'number'
      ? fixedNumber.round(precision).toString()
...
async function apy() {
  ...
  const rewarderInfos = await Promise.all(
    masterWombatPoolInfos.output.map(async ({ output: { rewarder } }) => {
      ...
      return sdk.api.abi.multiCall({
        ...
        chain,
        calls: Array(+rewardLength.output)
          .fill()
          .map((empty, idx) => ({
            target: rewarder,
            params: [idx],
  ...
  const { pricesByAddress, pricesBySymbol } = await utils.getPrices(
    Array.from(
      new Set([
        ...underlyingTokens.output.map(({ output }) => output),
        ...rewarderInfos.flatMap((poolRewardInfos) =>
          !poolRewardInfos
            ? []
            : poolRewardInfos.output.map(({ rewardToken }) => rewardToken)
        ),
        wom,
        quo,
      ])
  ...
  const getTokenPrice = (tokenAddress) =>
    BigNumber.fromDecimal(
      pricesByAddress[tokenAddress.toLowerCase()]?.toString() || '0'
    );
  const calculateApr = ({ rewardRate, rewardToken, tvlToken, tvlBalance }) => {
  ...
  return voterProxyUnderlyingAmounts.output.flatMap(
    (voterProxyUnderlyingAmount, i) => {
      const underlyingTokenAddress = underlyingTokens.output[i].output;
      if (
        voterProxyUserInfos.output[i].output.amount === '0' ||
        !voterProxyUnderlyingAmount.output
      )
        return [];
      if (
        underlyingTokens.output[i].output.toLowerCase() ===
        '0xE85aFCcDaFBE7F2B096f268e31ccE3da8dA2990A'.toLowerCase()
      )
        return []; // disable aBNBc (ankr bnb)
      ...
      if (voterProxyUnderlyingAmount.output) {
        ...
        quollBalanceAmount = +BigNumber.from(quollBalance)
          .mul(getTokenPrice(underlyingTokenAddress))
          .div(multiplier18)
          .toDecimal();

        const { rewardRate, sumOfFactors } =
          masterWombatPoolInfos.output[i].output;
        const basePartition = BigNumber.from(masterWombatBasePartition.output);
        const boostedPartition = BigNumber.from(
          masterWombatBoostedPartition.output
        );
        ...
        const boostedWomRewardRate = BigNumber.from(rewardRate)
        ...
        if (rewarderInfos[i]) {
          poolRewardInfos = rewarderInfos[i].output.flatMap((rewardInfo) => {
            const {
              output: { rewardToken, tokenPerSec },
            } = rewardInfo;
        ...
        const currentCliff = BigNumber.from(quoTotalSupply.output)
          .mul(cliffCount)
          .div(quoMaxSupply.output)
          .add(quoTotalSupply.output !== quoMaxSupply.output ? 1 : 0); // add 1 for Math.ceil
        const quoRewardRateMultipler = BigNumber.from(multiplier18)
          .mul(quoFactor.output)
          .div(quoFactorDenominator.output)
          .mul(cliffCount.sub(currentCliff))
          .div(cliffCount);
        const baseQuoRewardRate = baseWomRewardRate
          .mul(quoRewardRateMultipler)
          .div(multiplier18);
        const boostedQuoRewardRate = boostedWomRewardRate
          .mul(quoRewardRateMultipler)
          .div(multiplier18);

        const masterWombatUnderlyingAmount =
          masterWombatUnderlyingAmounts.output[i].output.amount;
        apr = apr
          .add(
            calculateApr({
