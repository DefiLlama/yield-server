...
async function callGetReservesData() {
	...
	let variableCompounded = calculateCompoundedInterest(wxdai.variableBorrowRate, latest_block.timestamp, wxdai.lastUpdateTimestamp)
	let stableCompounded = calculateCompoundedInterest(wxdai.averageStableRate, latest_block.timestamp, wxdai.stableDebtLastUpdateTimestamp)

	// Needed to calc scaled -> non-scaled variable debt
	let bigVariableBorrowIndex = BigNumber(wxdai.variableBorrowIndex);

	let totalVariableDebt = rayMul(BigNumber(wxdai.totalScaledVariableDebt), bigVariableBorrowIndex);
	let totalStableDebt = BigNumber(wxdai.totalPrincipalStableDebt)

	if(variableCompounded.gt(0)) totalVariableDebt = rayMul(totalVariableDebt, variableCompounded);
	if(stableCompounded.gt(0)) totalStableDebt = rayMul(totalStableDebt, stableCompounded);
	...
	return [{
		...
		tvlUsd: totalVariableDebt
			...
			.times(BigNumber(wxdai.priceInMarketReferenceCurrency).div(10 ** USD_DECIMALS))
			.div(10 ** WEI_DECIMALS)
			.toNumber(),
		apy: utils.aprToApy(parseFloat(BigNumber(wxdai.liquidityRate)
			.div(10 ** RAY_DECIMALS)
			.times(100)
		))
	}]
