...
async function apiCall(url, method, args = {}) {
  const result = await axios({
    method: 'get',
    url: indexerUrl + method,
    params: args
  })
  if (result.data.error) {
    throw new Error(`${result.data.error.message}: ${result.data.error.data}`)
...
function getPoolsByIds(poolIds) {
  ...
  if (!ids) return [];
  return apiCall(indexerUrl, 'list-pools-by-ids', {ids})
}
...
function getTokenPrices() {
  return apiCall(indexerUrl, 'list-token-price')
}
...
async function getV2SeedFarmsPools() {
  ...
  const list_farms = await Promise.all(farmsPromiseList);
  // get all pools
