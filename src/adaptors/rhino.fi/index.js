...
const getTokenSlug = (address, chain, coingeckoSlug) =>
  `${coingeckoSlug ? 'coingecko' : chain.toLowerCase()}:${
    coingeckoSlug || address.toLowerCase()
  }`;

const getTokenPrices = async (tokens) => {
  const l1TokenQuery = tokens.map(({ address, chain, coingeckoSlug }) =>
    getTokenSlug(address, chain, coingeckoSlug)
  );
  ...
  return Object.fromEntries(
    tokens.map(({ address, chain, token, coingeckoSlug }) => {
      const {
        ...
        price = 0,
        symbol = token,
      } = data.coins[getTokenSlug(address, chain, coingeckoSlug)] || {};
...
const findBeefyPrice = (beefyId, vaults) => {
  const { pricePerFullShare = 0, tokenDecimals: decimals = 18 } =
    vaults.find(({ id }) => id === beefyId) || {};
...
const getApy = async () => {
  ...
  const tvlPerChain = (
    await Promise.all(
      Object.keys(YIELD_TOKENS).map((chain) => {
  ...
  const usdPrices = (
    await Promise.all(
      Object.keys(YIELD_TOKENS).map((chain) =>
        getTokenPrices(YIELD_TOKENS[chain])
      )
    )
  ...
  return Object.values(YIELD_TOKENS)
    ...
    .map(({ address, chain, token, beefyId }, index) => {
      ...
      const { price: usdPrice, decimals } = beefyId
        ? findBeefyPrice(beefyId, beefyUsdPrices)
        : usdPrices[address] || { price: 0, decimals: 18 };
