...
const getApy = async () => {
  ...
  const pools = yieldPools.map((pool, i) => {
    const totalSupplyUsd = calculateTvl(pool.supply, pool.price, pool.decimals);
    const totalBorrowUsd = calculateTvl(pool.borrow, pool.price, pool.decimals);

    const tvlUsd = totalSupplyUsd - totalBorrowUsd;

    const borrowAPR = ((pool.rate * YEAR) / weiDecimals) * 100;
    const lendingAPR =
      ((pool.rate * YEAR * pool.utilization) / weiDecimals) * 100;
