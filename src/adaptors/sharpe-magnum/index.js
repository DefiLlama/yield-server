...
const { ChainId, Token, Fetcher } = require('@defillama/sdk5');
...
async function apy(vaultAddress) {
  ...
  if (leverage == 2) {
    return 3.0 * sRate - 2.0 * parseFloat(newBrate.toString());
  }
  // if vault working on 2x leverage
  else if (leverage == 1) {
    return 2.0 * sRate - parseFloat(newBrate.toString());
  }
...
async function getTVLInUSD(vaultAddress) {
  ...
  const response = await fetch(
    `https://coins.llama.fi/prices/current/${address}`
  );
