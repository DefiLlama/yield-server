...
var API_URL = 'https://api-mainnet.solidly.com/api/v1/pairs';
var SUBGRAPH_URL = 'https://api.thegraph.com/subgraphs/name/0xc30/solidly';
...
var getApy = function () { return __awaiter(_this, void 0, void 0, function () {
    var poolsRes, apyDict, alreadySeen, priceApiCoins, _i, poolsRes_1, pool, pairs, _a, pairs_1, pair, token0Key, token1Key, fullCoin, chunkSize, i, chunk, coins, pools;
        ...
        switch (_b.label) {
            ...
            case 2:
                ...
                for (_a = 0, pairs_1 = pairs; _a < pairs_1.length; _a++) {
                    pair = pairs_1[_a];
                    token0Key = 'ethereum:' + pair.token0.id.toLowerCase();
                    token1Key = 'ethereum:' + pair.token1.id.toLowerCase();
            ...
            case 6:
                pools = pairs.map(function (pair) {
                    ...
                    if (fullCoin['ethereum:' + pair.token0.id.toLowerCase()] && fullCoin['ethereum:' + pair.token1.id.toLowerCase()]) {
                        var token0ValueInReserve = parseFloat(pair.reserve0) * parseFloat(fullCoin['ethereum:' + pair.token0.id.toLowerCase()].price);
                        var token1ValueInReserve = parseFloat(pair.reserve1) * parseFloat(fullCoin['ethereum:' + pair.token1.id.toLowerCase()].price);
