...
const getTvlPerLSD = async (ticker) => {
  ...
  let savETHPool = response.liquidStakingNetworks[0].savETHPool,
    tvl = ethers.BigNumber.from('0');
  if (
    response.protectedDeposits.length != 0 ||
    response.feesAndMevDeposits.length != 0
  ) {
    for (let i = 0; i < response.protectedDeposits.length; ++i) {
      tvl = tvl.add(
        ethers.BigNumber.from(response.protectedDeposits[i].totalDeposit)
      );
    }
    let feesAndMevtotalDeposit = ethers.BigNumber.from('0');
    for (let i = 0; i < response.feesAndMevDeposits.length; ++i) {
      tvl = tvl.add(
        ethers.BigNumber.from(response.feesAndMevDeposits[i].totalDeposit)
      );
      feesAndMevtotalDeposit = feesAndMevtotalDeposit.add(
        ethers.BigNumber.from(response.feesAndMevDeposits[i].totalDeposit)
      );
    }
...
const topLvl = async (chainString, url, underlying) => {
  ...
  ethUsd =
    ethUSDPrice.coins['ethereum:0x0000000000000000000000000000000000000000']
      .price;
  ...
  let totalApy = 0,
    noOfActiveLSDs = 0;
  totalTvl = 0;
  await Promise.allSettled(promiseArray).then(async (result) => {
    for (let i = 0; i < result.length; ++i) {
      tvlUsd =
        ethers.utils.formatEther(result[i].value.tvl.toString()) * ethUsd;
