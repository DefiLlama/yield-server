...
// abis
const abi = {
  getSmartVaultAssetBalances:
    'function getSmartVaultAssetBalances(address, bool) external returns (uint256[] memory)',
  getAmountsOut:
    'function getAmountsOut(uint256, address[] memory) external view returns (uint256[] memory)',
  rewardConfiguration:
    'function rewardConfiguration(address, address) public view returns(uint32,uint32,uint192,uint32)',
  assetToUsd:
    'function assetToUsd(address, uint256) public view returns(uint256)',
};
...
const getTvlUsdRaw = async () => {
  const tvlETH = await call(spoolLens, abi['getSmartVaultAssetBalances'], [
    vault,
    false,
  ]);
  const ethPrice = await call(usdPriceFeedManager, abi['assetToUsd'], [
    weth,
...
const getApyBase = async () => {
  const apyBase = (
    await request(subgraphUrl, q, { smartVault: vault })
  ).smartVaultStrategies.reduce(
    (a, b) => a + (b.strategy.apy * b.allocation) / 100,
    0
  );
  return apyBase;
};
...
const getApyReward = async (tvlUsdRaw) => {
  const [rewardsDuration, periodFinish, rewardRate, tokenAdded] = await call(
    rewardManager,
    abi['rewardConfiguration'],
    [vault, spool]
  );
  ...
  const spoolPrice = await call(uniswapRouterV2, abi['getAmountsOut'], [
    constants.WeiPerEther.toString(),
    [spool, dai],
  ]);
  const spoolYearlyDistributionUsdRaw = BigNumber.from(rewardRate)
