...
const getApy = async () => {
  ...
  const poolFee = (
    await sdk.api.abi.multiCall({
      ...
      abi: abiPairFactory.find(
        (m) => m.name === 'getFee' && m.inputs.length === 1
      ),
      chain,
    })
  ).output.map((o) => o.output);
  ...
  const mpTokenPermutation = mpPooledTokens
    .map((tokenArr, i) =>
      tokenArr.map((_, j) => ({ multipoolIndex: i, tokenIndex: j }))
    )
    .flat();
  ...
  (
    ...
    .map((o) => o.output)
    .forEach((balance, i) => {
      mpReserves[mpTokenPermutation[i].multipoolIndex][
        mpTokenPermutation[i].tokenIndex
      ] = balance;
    });
  ...
  const poolLpToken = pools.map((pool, i) =>
    !poolIsMultipool[i] ? pool : mpLpToken[multipools.indexOf(pool)]
  );
  ...
  const poolGaugeStakingRatio = poolLpInGaugeBalance.map(
    (balance, i) => balance / poolLpTotalSupply[i]
  );
  ...
  const poolDescriptorsOfPairs = pairs.map((pair, i) => {
    ...
    const apyReward =
      poolGaugeStakingRatio[poolIndex] > 0 && tvlUsd > 0
        ? (((rewardRate / 1e18) * 86400 * 365 * rewardTokenPriceUsd) /
            tvlUsd /
            poolGaugeStakingRatio[poolIndex]) *
          100
    ...
    if (DEBUG) {
      pool = {
        ...
        rewardPerYearUsd:
          (rewardRate / 1e18) * 86400 * 365 * rewardTokenPriceUsd,
        rewardPerYearUsdToTvlRatioPercent:
          (((rewardRate / 1e18) * 86400 * 365 * rewardTokenPriceUsd) / tvlUsd) *
          100,
        rewardPerYearUsdToStakedTvlRatioPercent:
          (((rewardRate / 1e18) * 86400 * 365 * rewardTokenPriceUsd) /
            tvlUsd /
            poolGaugeStakingRatio[poolIndex]) *
          100,
  ...
  const poolDescriptorsOfMultipools = multipools.map((multipool, i) => {
    ...
    let tvlUsd = 0;
    mpPooledTokens[i].forEach(
      (tokenAddr, j) =>
        (tvlUsd +=
          (mpReserves[i][j] / 10 ** decimalsOf[tokenAddr]) *
    ...
    const customSymbol = mpPooledTokens[i]
      .map((tokenAddr) => mpUnderlyingTokenAddrToSymbol[tokenAddr])
      .join('-');
  ...
  return [...poolDescriptorsOfPairs, ...poolDescriptorsOfMultipools].filter(
    (p) => utils.keepFinite(p)
  );
};
