...
const getAPY = async (vaultAddy, chain) => {
  ...
  const apy =
    ((currPricePerShare - prevPricePerShare) / prevPricePerShare) * 100 * 52;
  return apy;
};
  ...
  for (const entry of Object.keys(vaults)) {
    for (const chain of Object.keys(vaults[entry])) {
      ...
      pools.push({
        ...
        tvlUsd: await getTokenPrice(
          `${chain}:${vaultParams[1]}`,
          Number((await getVaultState(vaults[entry][chain], chain))[1]),
          vaultParams[0]
        ),
