...
async function addTvlData(chain: string, vaults: Vault[]): Promise<Vault[]> {
  ...
  for (let j = 0; j < tvls.length; j++) {
    ...
    const tvlUsd = tvl
      ...
      .dividedBy(new bn(10).pow(newVault.asset0.decimals))
      .toNumber();
    newVault.tvl = tvl;
...
async function getAssetTvl(
  ...
  const globalStates = (
    await makeMulticall(globalStateFn, vaultAddresses, chain, null, {})
  ).outputByArray;
  ...
  const cycleStates = (
    await makeMulticall(cycleStateFn, vaultAddresses, chain, cycles, {})
  ).outputByArray;
