...
const {
  TEAHOUSE_VAULT_STAT_API_URL,
  TEAHOUSE_VAULT_CONTENT_API_URL,
  TEAHOUSE_WEBSITE_URL,
} = require('./config');
...
function mergeToVault(info: any, stat: any): Vault {
  ...
  const tokens = [];
  let rewardTokens = [];
  if (stat.latestInfo?.token0) tokens.push(getTokenData('0', info, stat));
  if (stat.latestInfo?.token1) tokens.push(getTokenData('1', info, stat));
...
async function getLiquidityData(
  ...
  const tvl = await makeCall(estimatedFnABI, vault.address, chain, null, {
    block: block,
  });
  const supplyFnABI = TEAHOUSE_VAULT_V3_ABI.find(
    (el) => el.name === `totalSupply`
  );
  const shareSupply = await makeCall(supplyFnABI, vault.address, chain, null, {
    block: block,
  });
  return {
    tvl: new bn(tvl || 0),
...
async function searchInterval(
  ...
  minInterval: number,
  check: (x: number) => Promise<boolean>
): Promise<number> {
...
async function addLiquidityData(
  vault: Vault,
  interval: number
): Promise<Vault> {
  const chain = vault.chain;
  ...
  const { tvl, shareSupply } = await getLiquidityData(vault);
  const before = Math.floor(Date.now() / 1000) - interval;
  const [blockBefore] = await utils.getBlocksByTime([before], chain);
  const { tvl: tvlBefore, shareSupply: shareSupplyBefore } =
    await getLiquidityData(vault, blockBefore);
  const prices = (await utils.getPrices([token.address], chain))
    .pricesByAddress;
  const tokenKey = token.address.toLowerCase();
  const tokenDecimals = new bn(10).pow(token.decimals);
  const tvlUsd = new bn(tvl || 0)
    .multipliedBy(prices[tokenKey])
    .div(tokenDecimals);
...
async function getRewardTokenApy(
  chain: string,
  ...
  const totalApr = rewardTokens.reduce((acc, el) => {
    ...
    const totalRewardUsd = reward
      .multipliedBy(prices[el.address.toLowerCase()])
      .div(new bn(10).pow(el.decimals));
...
async function updateBeforeLiquidityData(
  vault: Vault,
  time: number
): Promise<Vault> {
  const chain = vault.chain;
...
function calculateAPY(price: bn, priceBefore: bn, interval: number): number {
  ...
  const diffPercentYear = diffPercent
    .dividedBy(interval)
    .multipliedBy(365 * 24 * 60 * 60);
...
async function topLvl(_: number): Promise<Pool[]> {
  ...
  for (let vault of vaults) {
    ...
    const sharePriceBefore = calculateSharePrice(
      vault.tvlBefore,
      vault.shareSupplyBefore,
      decimals
    );
