...
const EVENTS = {
  DonatedRewards: 'event DonatedRewards(address indexed account, uint256 amount)',
};
...
const apy = async (timestamp: number | null = null) => {
  ...
  const events = await sdk.getEventLogs({
    ...
    eventAbi: EVENTS.DonatedRewards,
    fromBlock: startBlock,
    toBlock: currentBlock,
    chain: CHAIN,
  });

  let annualStakerEarnings = 0;
  if (events.length > 0) {
    const firstEventBlock = Number(events[0].blockNumber);
    const annualMultiplier = BLOCKS_PER_YEAR / (currentBlock - firstEventBlock);
    const totalBn = events.reduce(
      (acc, event) => acc + BigInt(event.args.amount),
      BigInt(0)
    );
    const total = Number(totalBn) / 1e18;
    annualStakerEarnings = total * annualMultiplier;
  }
