...
async function lpBalances() {
  ...
  const balances = totalSupplies.map((totalSupply, i) =>
    totalSupply.minus(withdrawals[i])
  );
...
async function getAprData() {
  return await fetchURL(
    'https://auto-rewards-prod-bucket-pool-stats-api-tokemakxyz.s3.amazonaws.com/current.json'
  );
}
...
function poolApr(aprData, reactor) {
  const apr = aprData.find(
    ({ address }) => address.toLowerCase() === reactor.poolAddress.toLowerCase()
  )?.liquidityProviderApr;
...
async function main() {
  ...
  const pools = reactors.map((reactor, i) => ({
    ...
    tvlUsd: calculateTvl(
      balances[i],
      reactor.decimals,
      prices[reactor.symbol.toLowerCase()]
    ),
