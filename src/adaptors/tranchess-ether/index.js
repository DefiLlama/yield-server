...
const topLvl = async (chainString, token, address) => {
    ...
    const totalUnderlying = (await sdk.api.abi.call({
      target: ethFund,
      abi: abi.getTotalUnderlying,
      chain: 'ethereum',
    })).output

    const equivalentTotalQ = (await sdk.api.abi.call({
      target: ethFund,
      abi: abi.getEquivalentTotalQ,
      chain: 'ethereum',
    })).output
    
    apy = (totalUnderlying / equivalentTotalQ - 1) / daysDiff * 365
    apy = apy * 100

    const eth_token_data = (await utils.getData(`https://coins.llama.fi/prices/current/${ethFundUnderlying}`)).coins[ethFundUnderlying];

    const tvlUsd = totalUnderlying * eth_token_data.price / Math.pow(10, eth_token_data.decimals)
