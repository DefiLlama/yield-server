...
const getPoolInfo = async () => {
  ...
  let apyReward = parseFloat(
    parseFloat((usdRewardPerYear / tvlUsd) * 100).toFixed(2)
  );

  return {
    pool: `${contract_addresses['BNBpancake-farm']}-${utils.formatChain(
      'binance'
    )}`,
...
async function getUSDRewardPerYear() {
  ...
  let farmContract = new ethers.Contract(
    contract_addresses['BNBpancake-farm'],
    farmABI,
    provider
  );
...
async function getTVLUSD() {
  ...
  let provider = new ethers.providers.JsonRpcProvider(BINANCE_RPC_URL);
  let pancakeContract = new ethers.Contract(
    contract_addresses['BNBpancakeSwapLP'],
    erc20ABI,
    provider
  );
  let farmContract = new ethers.Contract(
  ...
  let percentageStaked =
    parseFloat(totalStaked) /
    denomination /
    (parseFloat(totalSupply) / denomination);

  let wbnbBalance = await wbnbContract.balanceOf(
    contract_addresses['BNBpancakeSwapLP']
  );

  let totalBalanceUSDInPancake =
    (parseFloat(wbnbBalance) / denomination) * 2 * bnbPrice;
