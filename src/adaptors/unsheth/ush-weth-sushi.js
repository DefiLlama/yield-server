...
const getPoolInfo = async () => {
  ...
  return {
    pool: `${contract_addresses['sushi-farm']}-${utils.formatChain(
      'ethereum'
    )}`,
...
async function getTVLUSD() {
  ...
  let sushiContract = new ethers.Contract(
    contract_addresses['sushiSwapLP'],
    erc20ABI,
    provider
  );
  let farmContract = new ethers.Contract(
  ...
  let wethBalance = await wethContract.balanceOf(
    contract_addresses['sushiSwapLP']
  );

  let totalBalanceUSDInSushi =
    (parseFloat(wethBalance) / denomination) * 2 * ethPrice;
