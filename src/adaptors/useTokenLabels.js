...
const allLabelsFile = './assets.json'
...
console.log('rootFolder', rootFolder + '/**/*.js')
...
function run() {
  ...
  glob(rootFolder + '/**/*.js', {}, async (e, files) => {
    console.log('js file count', files.length)
    // console.log(files)
...
function updateFile(file) {
  ...
  if (relativePath.startsWith('assets')) relativePath = './' + relativePath
  const requireStr = `const ADDRESSES = require('${relativePath}')\n`
  let fileStr = fs.readFileSync(file, 'utf-8')
  const importedAddresses = fileStr.includes('assets.json')
