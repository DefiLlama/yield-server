...
const getStrategyResolver = async (chainID, strategyAddress) => {
    const { data } = await axios.get(`https://raw.githubusercontent.com/Popcorn-Limited/defi-db/main/archive/descriptions/strategies/${chainID}.json`);
...
const getStratApys = async () => {
    const { data } = await axios.get("https://raw.githubusercontent.com/Popcorn-Limited/defi-db/main/apy-data.json");
    return data;
}

const getGaugeApys = async () => {
    const { data } = await axios.get("https://raw.githubusercontent.com/Popcorn-Limited/defi-db/main/gauge-apy-data.json");
    return data;
};

async function getTokenPrice(chain, token) {
    const { data } = await axios.get(`https://coins.llama.fi/prices/current/${chain}:${token}`);
...
const apy = async (timestamp = null) => {
    ...
    for (const [chain, chainId] of Object.entries(CHAIN_TO_ID)) {
        ...
        for (const vault of vaults) {
            const strategyResolver = await getStrategyResolver(chainId, vault.strategies[0]);
            const apyBase = getBaseApy(stratApys, chainId, strategyResolver, vault.assetAddress);
            if (!apyBase) {
                // no apy
                continue;
            }
            ...
            const tvl = totalAssets * await getTokenPrice(chain, vault.assetAddress) / 10 ** decimals;
            ...
            if (chain === "ethereum") {
                const gaugeData = Object.values(gaugeApys).find((gauge) => gauge.vault === vault.address);
                if (gaugeData) {
