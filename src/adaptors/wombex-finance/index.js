...
async function tokenToBUSD(token, amount_in, chain) {
  ...
  if (token in config[chain].manual_eps) {
    path = [token, config[chain].manual_eps[token], config[chain].busd];
  } else {
    path = [token, config[chain].busd];
  }
  try {
    result = (
  ...
  try {
    ...
    decs = (
      await sdk.api.abi.call({
        target: tokenOut,
        chain: chain,
        abi: CoinABI.find((n) => n.name === 'decimals'),
      })
    ).output;
    potentialWithdraw.amount = BigNumber(potentialWithdraw.amount)
      .multipliedBy(BigNumber(one_eth))
      .dividedBy(BigNumber(10).pow(BigNumber(decs)))
      .valueOf();
...
async function setApys(
  ...
  for (let i = 0; i < len; i++) {
    ...
    decs = (
      await sdk.api.abi.call({
        target: config[chain].busd,
        chain: chain,
        abi: CoinABI.find((n) => n.name === 'decimals'),
      })
    ).output;
    if (token === config[chain].wom) {
      rpy = BigNumber(rewardState.rewardRate).multipliedBy(
        BigNumber(secToYear)
      );
      ...
      if (mintRatio > 0) {
        wmxRate = BigNumber(factAmountMint)
          .multipliedBy(BigNumber(mintRatio))
          .dividedBy(BigNumber(10000));
      }
      wmxApr = BigNumber(wmxRate)
        .multipliedBy(BigNumber(wmxUsdPrice))
        .multipliedBy(BigNumber(100))
        .dividedBy(BigNumber(poolValuestvl))
        .dividedBy(BigNumber(10).pow(BigNumber(decs)));
      aprTotal = aprTotal.plus(wmxApr);
      aprs.push({
        token: config[chain].wmx,
        apr: wmxApr.valueOf(),
      });
    }
    usdPrice = await tokenToBUSD(token, one_eth, chain);
    apr = BigNumber(rewardState.rewardRate)
      .multipliedBy(BigNumber(secToYear))
      .multipliedBy(BigNumber(usdPrice))
      .multipliedBy(BigNumber(100))
      .dividedBy(BigNumber(poolValuestvl))
      .dividedBy(BigNumber(10).pow(BigNumber(decs)));
  ...
  return {
    tokenAprs: aprs,
    totalApr: aprTotal.valueOf(),
    wmxApr: wmxApr.valueOf(),
  };
}
...
async function apy_chain(chain) {
  ...
  for (let i = 0; i < len_pools; i++) {
    ...
    poolValues['tvl'] = BigNumber(poolValues.lpTokenBalance)
      .multipliedBy(BigNumber(poolValues.lpTokenPrice))
      .dividedBy(BigNumber(one_eth))
      .multipliedBy(BigNumber(config[chain].tvl_corr))
      .valueOf();

    //compute apys of lp(!) pools

    if (poolValues.tvl > 10) {
      const { tokenAprs, totalApr, wmxApr } = await setApys(
        crvRewards,
        wmxUsdPrice,
        mintRatio,
        BigNumber(poolValues.tvl)
          .dividedBy(BigNumber(config[chain].tvl_corr))
          .valueOf(),
        poolValues,
        chain
      );
    ...
    a.map((a) => {
      if (Number(formatEther(a.tvl)) > 0 && Number(a.wmxApr) > 0) {
        apy_export.push({
          ...
          rewardTokens: a.tokenAprs.map((b) => {
            return b.token;
          }),
