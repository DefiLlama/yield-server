...
async function quoteWithdrawtoBUSD(pool, tokenOut, amt, chain) {
    try {
        ...
        decs = (await sdk.api.abi.call({
            target: tokenOut,
            chain: chain,
            abi: CoinABI.find((n)=>n.name === 'decimals')
        })).output;
        potentialWithdraw.amount = (BigNumber(potentialWithdraw.amount).multipliedBy(BigNumber(one_eth)).dividedBy((BigNumber(10)).pow(BigNumber(decs)))).valueOf();
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (token === config[chain].wom) {
            ...
            wmxApr = BigNumber(wmxRate).multipliedBy(BigNumber(wmxUsdPrice)).multipliedBy(BigNumber(100)).dividedBy(BigNumber(poolValuestvl)).dividedBy(
                BigNumber(10).pow(BigNumber(decs))
            );
...
async function apy_chain(chain) {
    ...
    for (let i = 0; i < len_pools; i++) {
        ...
        poolValues['tvl'] = (BigNumber(poolValues.lpTokenBalance).multipliedBy(BigNumber(poolValues.lpTokenPrice)).dividedBy(BigNumber(one_eth)).multipliedBy(BigNumber(config[chain].tvl_corr))).valueOf();

        //compute apys of lp(!) pools

        if (poolValues.tvl > 10) {
            const { tokenAprs, totalApr, wmxApr } = await setApys(crvRewards, wmxUsdPrice, mintRatio, (BigNumber(poolValues.tvl).dividedBy(BigNumber(config[chain].tvl_corr))).valueOf(), poolValues, chain);
