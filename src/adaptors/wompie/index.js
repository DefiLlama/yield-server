  ...
  for (let info of magpieInfos) {
    ...
    for (let i = 0, l = magpieInfo.pools.length; i < l; i++) {
      ...
      const pool = {
        ...
        isNative:
          rawInfo.poolType === 'WOMBAT_POOL'
            ? rawInfo.wombatHelperInfo.isNative
            : false,
        wombatPoolType:
          rawInfo.poolType === 'WOMBAT_POOL' && rawInfo.isActive
            ? rawInfo.wombatV3Pool.poolType.replace(' ', '_').toUpperCase()
            : '',
        ...
        stakingToken:
          rawInfo.poolType === 'WOMBAT_POOL'
            ? rawInfo.wombatHelperInfo.depositToken
            : rawInfo.poolType === 'MAGPIE_VLMGP_POOL'
            ? mgpAddress
        ...
        isPoolFeeFree:
          rawInfo.poolType === 'WOMBAT_POOL' && rawInfo.isActive
            ? rawInfo.wombatStakingPool.isPoolFeeFree
            : false,
        rawStakingTokenInfo: {
        ...
        stakingTokenInfo:
          rawInfo.poolType === 'WOMBAT_POOL'
            ? {
                ...
                priceId:
                  rawInfo.wombatHelperInfo.depositTokenSymbol.toUpperCase(),
              }
            : {
                name: rawInfo.stakingTokenSymbol,
                symbol: rawInfo.stakingTokenSymbol,
      ...
      pool.tvlInfo = {
        ...
        totalAmount: new BigNumber(formatEther(rawInfo.sizeOfPool))
          .multipliedBy(
            priceData[pool.stakingTokenInfo?.showSymbol?.toUpperCase()]
          )
          .toNumber(),
      };
      if (rawInfo.isActive) {
        ...
        rewardAmount = rewardAmount
          ...
          .multipliedBy(60)
          .multipliedBy(60)
          .multipliedBy(priceData['MGP']);
        const mgpApr = rewardAmount.dividedBy(
          new BigNumber(pool.tvlInfo.totalAmount)
        );
        const mgpAprItem = {
          name: rawInfo.isMPGRewardPool ? 'MGP' : 'vlMGP',
          symbol: rawInfo.isMPGRewardPool ? 'MGP' : 'vlMGP',
          value: mgpApr.toNumber(),
          formatValue: mgpApr.gt(new BigNumber(0))
            ? mgpApr.multipliedBy(100).toFixed(2)
            : '0.0',
        ...
        if (pool.type === 'WOMBAT_POOL') {
          const poolFactor = Number(
            formatEther(rawInfo.wombatV3Pool.sumOfFactors)
          );
          const userBalance = Number(
            formatEther(rawInfo.wombatV3Pool.wombatStakingUserAmount)
          );
          const userfactor = Number(
            formatEther(rawInfo.wombatV3Pool.wombatStakingUserFactor)
          );
          const totalSupply = Number(
            formatEther(rawInfo.wombatV3Pool.totalSupply)
          );
          const rewardRate = Number(
            formatEther(rawInfo.wombatV3Pool.rewardRate)
          );
          const normPerSec = (rewardRate * 0.375 * userBalance) / totalSupply;
          const boostedPerSec = (rewardRate * 0.625 * userfactor) / poolFactor;
          const rewardAmount = new BigNumber(normPerSec + boostedPerSec)
            .multipliedBy(365)
            .multipliedBy(24)
            .multipliedBy(60)
            .multipliedBy(60)
            .multipliedBy(pool.isPoolFeeFree ? 1 : 0.8)
            .multipliedBy(priceData['WOM']);
          wombatRewardTokens[0].tokenPerSec =
            wombatRewardTokens[0].tokenPerSec + normPerSec + boostedPerSec;
          const womApr = rewardAmount.dividedBy(
            new BigNumber(pool.tvlInfo.totalAmount)
          );
          const womAprItem = {
            name: 'WOM',
            ...
            formatValue: womApr.gt(new BigNumber(0))
              ? womApr.multipliedBy(100).toFixed(2)
              : '0.0',
            address: womAddress,
          };
          pool.aprInfo.items.push(womAprItem);
          for (
            let i = 0, l = rawInfo.wombatV3Pool.rewardList.length;
            i < l;
            i++
          ) {
            ...
            const rewardAmount = new BigNumber(
              formatEther(rewardInfo.tokenPerSec)
            )
              .multipliedBy(365)
              .multipliedBy(24)
              .multipliedBy(60)
              ...
              .multipliedBy(pool.isPoolFeeFree ? 1 : 0.8)
              .multipliedBy(
                priceData[rewardInfo.rewardTokenSymbol.toUpperCase()]
              );
            const tvlAmount = new BigNumber(
              formatEther(rawInfo.wombatV3Pool.totalSupply)
            ).multipliedBy(
              priceData[
                rawInfo.wombatHelperInfo.depositTokenSymbol.toUpperCase()
            ...
            const rewardAprItem = {
              ...
              formatValue: rewardApr.gt(new BigNumber(0))
        ...
        pool.aprInfo.formatTotalApr = new BigNumber(pool.aprInfo.totalApr).gt(
          new BigNumber(0)
        )
          ? new BigNumber(pool.aprInfo.totalApr).multipliedBy(100).toFixed(2)
          : '0.0';
      }
    ...
    if (magpieWomPool && magpieWomPool.aprInfo) {
      for (let i = 0, l = wombatRewardTokens.length; i < l; i++) {
        ...
        if (rewardInfo.symbol === 'WOM') {
          apr = new BigNumber(rewardInfo.tokenPerSec)
            .multipliedBy(365)
            .multipliedBy(24)
            .multipliedBy(60)
            .multipliedBy(60)
          ...
          if (rewardInfo.userBalance && rewardInfo.totalSupply) {
            rewardAmount = rewardAmount
              .multipliedBy(new BigNumber(formatEther(rewardInfo.userBalance)))
              .dividedBy(new BigNumber(formatEther(rewardInfo.totalSupply)));
          }
          rewardAmount = rewardAmount.multipliedBy(
            priceData[rewardInfo.symbol.toUpperCase()]
          );
        ...
        magpieWomPool.aprInfo?.items.push({
          ...
          value: apr.toNumber(),
          formatValue: apr.gt(new BigNumber(0))
            ? apr.multipliedBy(100).toFixed(2)
            : '0.0',
      ...
      for (let i = 0, l = magpieWomPool.aprInfo.items.length; i < l; i++) {
        totalApr = totalApr.plus(
          new BigNumber(magpieWomPool.aprInfo?.items[i].value)
        );
      }
      magpieWomPool.aprInfo.totalApr = totalApr.toNumber();
      magpieWomPool.aprInfo.formatTotalApr = new BigNumber(
        magpieWomPool.aprInfo.totalApr
      ).gt(new BigNumber(0))
        ? new BigNumber(magpieWomPool.aprInfo.totalApr)
            .multipliedBy(100)
            .toFixed(2)
        : '0.0';
    }
    ...
    if (magpieVlMgpPool && magpieVlMgpPool.aprInfo) {
      ...
      for (let i = 0, l = magpieVlMgpPool.aprInfo.items.length; i < l; i++) {
        totalApr = totalApr.plus(
          new BigNumber(magpieVlMgpPool.aprInfo?.items[i].value)
        );
      }
      magpieVlMgpPool.aprInfo.totalApr = totalApr.toNumber();
      magpieVlMgpPool.aprInfo.formatTotalApr = new BigNumber(
        magpieVlMgpPool.aprInfo.totalApr
      ).gt(new BigNumber(0))
        ? new BigNumber(magpieVlMgpPool.aprInfo.totalApr)
            .multipliedBy(100)
            .toFixed(2)
        : '0.0';
    }
