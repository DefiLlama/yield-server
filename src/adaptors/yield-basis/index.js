...
// The market roster grows over time (11 markets as of 2026-09-12, several of
// them older versions of the same asset), so the list is read from the factory
// instead of being hardcoded.
const getMarkets = async () => {
  const marketCount = await sdk.api.abi
    .call({ target: FACTORY, abi: ABI.marketCount, chain: CHAIN })
    .then((res) => Number(res.output));

  if (!Number.isFinite(marketCount) || marketCount <= 0) return [];

  const ids = [...Array(marketCount).keys()];

  const markets = await multiCall({
    abi: ABI.markets,
    calls: ids.map((id) => ({ target: FACTORY, params: [id] })),
  });
...
const getUnstakedRows = ({
  ...
  markets
    .map((market, index) => {
      ...
      const symbol = ltSymbols[index] || `yb-${market.id}`;
