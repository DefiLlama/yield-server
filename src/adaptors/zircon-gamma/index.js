...
let gettingDailyVolume = async function (pairAddress, chainId) {
  ...
  let query = await axios.post(GAMMA_SUBGRAPH_URI[chainId], JSON.stringify({query: QUERY, variables: null, operationName: undefined} ), )
...
let calculatePrices = async function(chainId, web3) {
  ...
  const zrgNtvContract = new web3.eth.Contract(JSON.parse(PAIR_CONTRACT_ABI), NTV_ZRG_PAIR_ADDRESS[chainId]);
  const usdcMovrContract = new web3.eth.Contract(JSON.parse(PAIR_CONTRACT_ABI), NTV_USDC_PAIR_ADDRESS[chainId]);
  ...
  let zrgNtvPrice
  if (symbol0 === "ZRG") {
    zrgNtvPrice = new BigNumber(zrgNtvReserves[1].toString()).dividedBy(zrgNtvReserves[0].toString());
  }else{
    zrgNtvPrice = new BigNumber(zrgNtvReserves[0].toString()).dividedBy(zrgNtvReserves[1].toString());
  }
  ...
  if (symbol0USDC.startsWith("USD")) {
    ntvUsdPrice = new BigNumber(usdcMovrReserves[0].toString()).multipliedBy(new BigNumber("1e" + (decimals1USDC-decimals0USDC))).dividedBy(usdcMovrReserves[1].toString())
  }else{
    ntvUsdPrice = new BigNumber(usdcMovrReserves[1].toString()).multipliedBy(new BigNumber("1e" + (decimals0USDC-decimals1USDC))).dividedBy(usdcMovrReserves[0].toString())
  }
  // Hardcoded to 1e12 because USDC is 6 decimals and MOVR is 18 decimals
  return {zrg: zrgNtvPrice.multipliedBy(ntvUsdPrice), movr: ntvUsdPrice, ntv: ntvUsdPrice}
}
...
let getPools =  async function (chainId) {
  ...
  const zrgContract = new web3.eth.Contract(JSON.parse(PAIR_CONTRACT_ABI), ZRG_ADDRESS[chainId])
  const wmovrContract = new web3.eth.Contract(JSON.parse(PAIR_CONTRACT_ABI), WMOVR_ADDRESS[chainId])
  ...
  if (!_.isEmpty(pools)) {
    for (const pool of pools) {
      try {
        ...
        let rewardTokens = []
        if (ZRGBalance > 0) { rewardTokens.push(ZRG_ADDRESS[chainId]) }
        if (WMOVRBalance > 0) { rewardTokens.push(WMOVR_ADDRESS[chainId]) }
        ...
        const isFloatRes0 = await pylonContract.methods.isFloatReserve0().call();

        // Getting Info from Tokens
        const token0Contract = new web3.eth.Contract(JSON.parse(PAIR_CONTRACT_ABI), isFloatRes0 ? token0 : token1);
        const token1Contract = new web3.eth.Contract(JSON.parse(PAIR_CONTRACT_ABI), !isFloatRes0 ? token0 : token1);
        ...
        let movrEarningsPerBlock = new BigNumber(MOVRBalanceCorrected).multipliedBy(prices.movr.toString()).dividedBy(1e18).dividedBy(blocksRemaining.toString())
        let zrgEarningsPerBlock = new BigNumber(ZRGBalanceCorrected).multipliedBy(prices.zrg.toString()).dividedBy(1e18).dividedBy(blocksRemaining.toString())

        let earningPerYear = (new BigNumber(zrgEarningsPerBlock).plus(movrEarningsPerBlock)).multipliedBy(DAILY_BLOCK[chainId]*365);
